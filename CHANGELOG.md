...
- A figure for the wheel hand-off. The desktop section described it in a
  sentence, and it is the feature nobody knows to look for because both ways of
  getting it wrong fail silently: always claiming the wheel traps a reader in
  the post, never claiming it means no desktop zoom at all. Two feeds side by
  side say which one gets the event. Generated by
  `tool/wheel_handoff_figure.dart`.
